...
bool Image::load( const std::string filename ) {
    ...
    lodepng::load_file(m_buffer, filename);
    error = lodepng::decode(raw, width, height, m_state, m_buffer);
    if ( error ) return false;

    set(width, height);

    // Se determina que espacio de color es la imagen (RGB, GrayScale, ect)
    LodePNGInfo info = m_state.info_png;
...
bool Image::load_gray( std::vector<unsigned char> raw /* const std::string filename*/) {
    for ( int i=0; i<int(m_width*m_height); i++ ) {
        m_data[i].y = raw[i];
    }
